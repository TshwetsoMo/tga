...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Get the posted date from the form
    $postedDate = $_POST['postedDate']; // Should be in 'Y-m-d' format

    $query = "INSERT INTO announcements (title, description, postedDate, postedBy, role) VALUES (?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("sssss", $title, $description, $postedDate, $postedBy, $role);
    $stmt->execute();
...
<head>
    <meta charset="UTF-8">
    <title>Add Announcement - TechGenius Academy</title>
    <!-- Bootstrap CSS -->
